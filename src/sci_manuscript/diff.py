...
\AtBeginDocument{%
  \renewcommand{\review}[2]{%
    ...
    % Provenance blocks may contain headings, displays, floats, or paragraphs.
    % Keep these structurally transparent and use color instead of a ulem scan.
    {\RevisionReviewContexttrue
      \RevisionReviewBackground{{\RevisionReviewFont\color{RevisionReviewColor}#2}}}%
    \ReviewLineLabel{review:\ReviewBlockID:end}%
    \immediate\write\ReviewLocationFile{#1|\ReviewBlockID}%
  }%
  % Backward-compatible only: author additions require no wrapper and use the
  % ordinary latexdiff blue-wave convention.
...
def _parse_provenance_command(text: str, start: int) -> tuple[str, int] | None:
    for name, count in ((r"\review", 2), (r"\user", 1)):
