# Third-party notices

The repository MIT License applies to original workflow code, documentation,
tests, and original templates. It does not replace the terms attached to the
third-party LaTeX resources listed below.

## Elsevier `elsarticle`

Files:

- `src/resources/journal_templates/elsevier/elsarticle.cls`
- `src/resources/journal_templates/elsevier/elsarticle-num.bst`

The class and bibliography style retain Elsevier copyright notices and the
LaTeX Project Public License terms embedded in the upstream files. Provenance
and source links are recorded in the resource README.

## Springer Nature `sn-jnl`

Files:

- `src/resources/journal_templates/nature/sn-jnl.cls`
- `src/resources/journal_templates/nature/sn-nature.bst`

The class and bibliography style retain their upstream LaTeX Project Public
License notices. Provenance and source links are recorded in the resource
README.

## ACS `achemso`

Files:

- `src/resources/journal_templates/acs/achemso.cls`
- `src/resources/journal_templates/acs/achemso.dtx`

These files are distributed under the LaTeX Project Public License 1.3c or
later. The bundled `src/resources/journal_templates/acs/LICENSE.md` preserves the
upstream notice.

## Chinese numeric bibliography style `kxtbcas-numeric`

File:

- `src/resources/journal_templates/chinese/kxtbcas-numeric.bst`

This renamed generated bibliography style is derived from `achemso.bst` and is
governed by the upstream LPPL/generated-file terms. The corresponding upstream
source `achemso.dtx` is included under the ACS resources. The original
`kxtbcas.cls` is project-owned MIT-licensed work and is therefore not listed as
a third-party resource.
